...
cuda_get_frame_block (frame_info_ptr frame)
{
  CORE_ADDR pc = get_frame_address_in_block (frame);
  if (!pc)
    return nullptr;

  const struct block *frame_block = block_for_pc (pc);
  if (!frame_block)
    return nullptr;

  struct obj_section *section = find_pc_mapped_section (pc);
  struct compunit_symtab *cust = find_pc_sect_compunit_symtab (pc, section);
  if (!cust)
    return nullptr;

  const struct blockvector *block_vector = cust->blockvector ();
  if (!block_vector)
    return nullptr;

  /* skip the blocks of the functions inlined under this frame */
  int inline_count = frame_inlined_callees (frame);
  while (frame_block)
    {
      if (inline_count == 0 && frame_block->function ())
        break;
      if (frame_block->inlined_p ())
        inline_count--;
  ...
  const struct block *frame_function_block = frame_block;
  while (frame_function_block->function () == nullptr
         && frame_function_block->superblock () != nullptr)
    frame_function_block = frame_function_block->superblock ();
  if (!frame_function_block)
    return nullptr;

  /* iterate over all the blocks */
  const struct block *best_block = nullptr;
  for (const block *b : block_vector->blocks ())
    {
      /* discard the blocks that do not contain the PC */
      if (b->start () > pc || b->end () <= pc)
        continue;

      /* discard the blocks that are wider than the current best block */
      if (best_block && best_block->start () > b->start ())
        continue;
      if (best_block && best_block->start () == b->start ()
          && best_block->end () < b->end ())
        continue;

      /* discard block whose parent function block is not our frame function
         block */
      const struct block *function_block = b;
      while (function_block->function () == nullptr
             && function_block->superblock () != nullptr)
