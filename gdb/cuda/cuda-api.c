...
cuda_debugapi::cuda_api_error(CUDBGResult res, const char *fmt, ...)
{
  ...
  va_start (args, fmt);
  vsnprintf (errStr.data (), errStr.size (), fmt, args);
  va_end (args);
  ...
  throw_error (GENERIC_ERROR, "Error: %s, error=%s, error message=%s\n",
	       errStr.data (), cudbgGetErrorString (res), errStrEx.data ());
}
...
{
  constexpr size_t num_chars = sizeof (m->mask) * 2;
  static char buf[num_chars + 3] = { 0 };
  sprintf (buf, "0x%.*llx", (int)num_chars, (unsigned long long)(m->mask));
  return buf;
}
...
cuda_debugapi::read_local_memory (uint32_t dev, uint32_t sm, uint32_t wp, uint32_t ln, uint64_t addr, void *buf, uint32_t sz)
{
  ...
  CUDA_API_TRACE_DEV_SM_WARP_LANE (dev, sm, wp, ln, "0x%lx size %u", addr, sz);
...
void
cuda_debugapi::get_cbu_warp_state (uint32_t dev, uint32_t sm,
				   const cuda_api_warpmask &wp_mask,
				   CUDBGCbuWarpState *states,
				   uint32_t numStates)
{
  gdb_assert (states);
  gdb_assert (numStates);

  if (!api_state_initialized ())
    return;

  CUDA_API_PROFILE (getCbuWarpState);

  memset (states, 0, sizeof (*states) * numStates);

  CUDBGResult res = CUDBG_ERROR_NOT_SUPPORTED;
  if (api_version ().m_revision >= 153)
    res = s_instance.m_cudbgAPI->getCbuWarpState (dev, sm, wp_mask.mask,
						  states, numStates);
  cuda_api_print_api_call_result (__FUNCTION__, res);
  ...
  CUDA_API_TRACE_DEV_SM (dev, sm,
			 "wp_mask %" WARP_MASK_FORMAT " requested states %u",
			 cuda_api_mask_string (&wp_mask), numStates);
}
		   ...
		   uint32_t *numConsumed)
{
  ...
  CUDA_API_PROFILE (consumeCudaLogs);

  memset (logMessages, 0, sizeof (*logMessages) * numMessages);
  *numConsumed = 0;
