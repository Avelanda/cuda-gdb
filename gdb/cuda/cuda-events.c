...
static void
cuda_event_kernel_ready (uint32_t dev_id, uint64_t context_id,
			 uint64_t module_id, uint64_t grid_id, uint32_t tid,
			 uint64_t virt_code_base, CuDim3 grid_dim,
			 ...
			 uint64_t parent_grid_id, CUDBGKernelOrigin origin)
{
  ...
  cuda_trace_event ("CUDBG_EVENT_KERNEL_READY dev_id=%u context=0x%lx"
		    " module=0x%lx grid_id=%ld tid=%u type=%u"
		    " parent_grid_id=%ld",
  ...
  const auto kernel = cuda_state::create_kernel (
      ...
      CuDim3{ 0 }, type, origin, parent_grid_id);
