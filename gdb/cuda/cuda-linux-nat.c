...
cuda_alloc_blockvector (struct symtab *symtab, int nblocks)
{
  ...
  struct block *g_block = new (obstack) global_block;
  g_block->set_multidict (
      mdict_create_hashed_expandable (symtab->language ()));
  bv->set_block (GLOBAL_BLOCK, g_block);

  /* Only allowed for the GLOBAL block */
  g_block->set_compunit_symtab (symtab->compunit ());

  /* Allocate the STATIC block*/
  struct block *static_block = new (obstack) struct block;
  ...
  static_block->set_superblock (g_block);
