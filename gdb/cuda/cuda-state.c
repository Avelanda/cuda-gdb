...
cuda_state::reset (void)
{
  ...
  // Modules belong to contexts, so delete them first
  CUDA_STATE_TRACE ("Unloading remaining objfiles");
  for (auto &iter : m_instance.m_module_map)
    {
      auto module = iter.second.get ();
      module->context ()->remove_module (module);
      module->unload_objfile (true);
    }

  CUDA_STATE_TRACE ("Destroying module map");
  m_instance.m_module_map.clear ();

  // Delete the kernel map (and the associated kernel objects)
  CUDA_STATE_TRACE ("Destroying kernel map");
  m_instance.m_kernel_map.clear ();
  m_instance.m_next_kernel_id = 0;

  CUDA_STATE_TRACE ("Destroying context map");
  m_instance.m_context_map.clear ();

  // Automatically deletes all device/sm/warp/lane objects
  CUDA_STATE_TRACE ("Destroying Device/SM/Warp/Lane objects");
...
cuda_state::destroy_module (uint64_t module_id)
{
  ...
  // Unload and request destruction of the objfile
  module->unload_objfile (true);

  // Remove the module from the module map, destroying it
...
cuda_state::find_module_by_id (uint64_t module_id)
{
  ...
  if (iter != m_instance.m_module_map.end ())
    {
      auto module = iter->second.get ();
      CUDA_STATE_TRACE ("Module 0x%llx found %p", module_id, module);
      return module;
    }

  CUDA_STATE_TRACE ("Module 0x%llx not found", module_id);
  return nullptr;
}
...
cuda_state::find_kernel_by_grid_id (uint32_t dev_id, uint64_t grid_id)
{
  for (const auto& iter : m_instance.m_kernel_map)
	...
	CUDA_STATE_TRACE_DEV (device (dev_id),
			      ...
			      iter.second.get ()->id (), (int64_t)grid_id);
	return iter.second.get ();
      }
...
void
cuda_state::update_kernels_terminated (void)
{
...
cuda_device::invalidate (bool quietly, bool recurse)
{
  ...
  cuda_state::invalidate_kernels (dev_idx ());
...
cuda_device::get_kernel (uint64_t grid_id)
{
  ...
  auto kernel = cuda_state::find_kernel_by_grid_id (dev_idx (), grid_id);
  if (!kernel)
    kernel = cuda_state::create_kernel (dev_idx (), grid_id);
			   ...
			   cuda_api_warpmask *single_stepped_warp_mask)
{
  ...
  if (!warp (wp_id)->valid ())
    ...
    {
      const auto valid_lanes_mask = warp (wp_id)->get_valid_lanes_mask ();
...
const CUDBGCbuWarpState &
cuda_warp::get_cbu_state ()
{
  update_cbu_state ();
  return m_cbu_state;
}
		 ...
		 const CUDBGDeviceInfo *dev_info, uint8_t *&ptr)
{
  ...
  cuda_api_warpmask updated_warp_mask
